...
def constructBayesNet(gameState):
    ...
    totalHouses =  gameState.getPossibleHouses()
    for eachHouse in totalHouses:
        allwalls = gameState.getHouseWalls(eachHouse)
        for eachWall in allwalls:
            var = OBS_VAR_TEMPLATE % eachWall
            obsVars.append(var)
    ...
    for eachHouse in totalHouses:
        ...
        for eachWall in allwalls:
            var = OBS_VAR_TEMPLATE % eachWall
            variableDomainsDict[var] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    houseCounter = 0
    allHousePositions = gameState.getPossibleHouses()
    possHouseVals = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    for eachHouse in allHousePositions:
        allHouseWalls =  gameState.getHouseWalls(eachHouse)
        for eachWall in allHouseWalls:
            eachWall = OBS_VAR_TEMPLATE % eachWall
            cFactor = bn.Factor([eachWall], HOUSE_VARS, bayesNet.variableDomainsDict())
            for assignment in cFactor.getAllPossibleAssignmentDicts():
                prob = 0
                currAssignmnt = assignment[eachWall]
                foodAssignment = assignment[FOOD_HOUSE_VAR]
                ghostAssignment = assignment[GHOST_HOUSE_VAR]
                curHouseVal = possHouseVals[houseCounter]
                if foodAssignment == curHouseVal:
                    prob = getProb(currAssignmnt,prob,"food")
                elif ghostAssignment == curHouseVal:
                    prob = getProb(currAssignmnt, prob,"ghost")
                elif currAssignmnt == NO_OBS_VAL:
                    prob = 1
                cFactor.setProbability(assignment, prob)
            bayesNet.setCPT(eachWall, cFactor)
        housetemp = houseCounter + 1
        houseCounter = housetemp % 4

#get probability for food, ghost based on flag
def getProb(currAssignmnt,prob,flag):
    if currAssignmnt == RED_OBS_VAL:
        if flag == "food":
            prob = PROB_FOOD_RED
        elif flag == "ghost":
            prob = PROB_GHOST_RED
    elif currAssignmnt == BLUE_OBS_VAL:
        if flag == "food":
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    allAssignments = varelimination.getAllPossibleAssignmentDicts()
    return getMaxProb(allAssignments,varelimination)

def getMaxProb(allAssignment,varelimination):
    probMax = (0,0)
    for eachassignment in allAssignment:
        prob = varelimination.getProbability(eachassignment)
        if prob > probMax[0]:
            probMax = (prob, eachassignment)
    return probMax[1]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        CPT = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence,
                                                       eliminationOrder)
        #topright
        tr = self.getCorner(evidence,2)

        #topleft
        tl = self.getCorner(evidence,1)

        #leftexpected
        gamewon = CPT.getProbability(tl)*WON_GAME_REWARD
        ghostcol = CPT.getProbability(tr)*GHOST_COLLISION_REWARD
        leftExpectedValue = gamewon + ghostcol

        #rightexpected
        gamewon2 = CPT.getProbability(tl) * GHOST_COLLISION_REWARD
        ghostcol2 = CPT.getProbability(tr) * WON_GAME_REWARD
        rightExpectedValue = gamewon2 + ghostcol2

        return leftExpectedValue, rightExpectedValue

    def getCorner(self,evidence,flag):
        corner = {}
        if flag == 1:
            corner.update(evidence)
            corner[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
            corner[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        elif flag == 2:
            corner.update(evidence)
            corner[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
            corner[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        return corner
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        poutcome = self.getExplorationProbsAndOutcomes(evidence)
        for(p,e) in poutcome:
            maxp = max(self.computeEnterValues(e,enterEliminationOrder))
            expectedValue += p*maxp
