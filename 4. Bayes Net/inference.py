...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        factors = getFactors(eliminationOrder,factors,eliminate,joinFactorsByVariable)
        alljoint = joinFactors(factors)
        normalizedJoin = normalize(alljoint)
        return normalizedJoin
    return inferenceByVariableElimination
inferenceByVariableElimination = inferenceByVariableEliminationWithCallTracking()

def getFactors(eliminationorder,factors,eliminate,joinFactorsByVariable):
    for eachvar in eliminationorder:
        factors, joinedFactor = joinFactorsByVariable(factors, eachvar)
        joinedFactorLen = len(joinedFactor.unconditionedVariables())
        if joinedFactorLen > 1:
            curJoint = eliminate(joinedFactor, eachvar)
            factors.append(curJoint)
    return factors
